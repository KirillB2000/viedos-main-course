...
describe("Videos API", () => {
  ...
  it("should update video by id; UPDATE /videos/:id", async () => {
    ...
    expect(getResponse.body.canBeDownloaded).toBe(
      updateVideoInputData.canBeDownloaded,
    );
    expect(getResponse.body.minAgeRestriction).toBe(
      updateVideoInputData.minAgeRestriction,
    );
    expect(getResponse.body.publicationDate).toBe(
      updateVideoInputData.publicationDate,
    );
    expect(getResponse.body.availableResolutions.join()).toBe(
      updateVideoInputData.availableResolutions.join(),
    );
  });
